...
- 跨服务复用的纯/半纯 helper(schemas、推荐引擎、preview 构造、candidate 评估、metadata 工具)
  放在 `src/server/services/internal/`,由各服务模块 import。
- `world-model-services.ts` 只作为**组合根**(composition root):装配各 `createXxxService(ctx)` 并返回
  `WorldModelServices`,不承载具体业务实现。
- 经验阈值:单个服务模块尽量 **< 600 行**;`internal/` helper 模块尽量 **< 400 行**。超出说明该聚合需要再拆。
