...
- `observations`：写入观察、去重、标记未知/重复/拒绝/确认、结算；低影响观察留在未知证据队列，支持单条/批量拒绝。
...
- `sources`：来源配置与 preset、运行采集 adapter、保存 observation run、失败/低增量来源抑制。
- `automation`：证据自动闭环（采集→去重→候选→确认→更新）、心跳、worker 配置。
...
- LLM 主评分、轻量 fallback 加载、estimator 装配在 `src/server/models/estimators.ts`，由 `configured.ts`
  注入到组合根的 `options`。
- **后续目标**：把组合根中仍然耦合的有状态闭包（证据确认/应用/回滚、来源运行、证据闭环）进一步拆成
  `belief-service.ts`、`observation-service.ts`、`evidence-service.ts`、`update-service.ts`、
  `source-service.ts`、`automation-service.ts` 等 per-service 工厂模块，由组合根注入共享上下文装配。
  当前已先完成 `internal/` helper 层抽离与纯查询逻辑下沉。
