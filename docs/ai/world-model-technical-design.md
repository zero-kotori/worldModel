...
- **领域核心**：`src/domain` 承载贝叶斯更新、互斥/独立概率更新、似然 ensemble 和去重决策，保持无 IO、无框架依赖。
- **纯 UI 适配层**：`src/lib` 承载图谱布局、页面视图模型、状态文案、来源 preset 和 action 路由 helper。
- **服务层**：`src/server/services` 已按信念、观察、证据、似然、更新、来源、自动化、模型 8 个聚合拆分，`world-model-services.ts` 只作为组合根装配 `WorldModelStore` 和 estimator/options。
- **自动闭环**：来源采集会经过 Query Planner、来源选择、去重、候选推荐、低影响/未匹配分流、自动应用保护和 observation run 审计。
- **操作界面**：后台保留总览、图谱、信念、观察、证据、来源、模型 7 个主 tab。图谱页是唯一完整关系图谱工作区，其他页面只保留必要入口或聚焦视图。
- **人工清理策略**：重复候选默认拒绝；未知观察和低相关性观察可以在单次闭环或 worker 配置中选择保留、拒绝或软删除。
- **模型与训练**：LLM 是主评分器，轻量模型和外部 deep-model adapter 是可选辅助；训练、评估和导入产物写入 `model-artifacts/`，不提交 Git。
...
worldModel/
  ...
  src/
    ...
    server/
      ...
      proxy-auth.ts
      services/
        types.ts            # WorldModelStore / WorldModelServices 接口与记录类型
        world-model-services.ts  # createWorldModelServices 组合根
        in-memory-store.ts  # 内存实现（测试/本地闭环）
        prisma-store.ts     # Prisma 实现
        configured.ts       # 装配 estimator/options 后的服务工厂
        index.ts            # getWorldModelServices()（prisma + configured）
        internal/           # 跨服务共享 helper（schemas/shared/recommendations/evidence-queries/model-artifact）
      sources/
      models/
      training/
      automation/
...
`DELETED` 是软删除状态，用于从操作队列和图谱中隐藏噪声观察，同时保留审计痕迹；已删除观察不能再确认为证据。`SETTLED` 用于已经消费到假设结算流程的观察。
...
自动闭环和 worker 默认把重复候选清理为 `REJECTED`，因为重复项通常不需要继续进入人工队列。未知观察和低影响观察默认保留，但表单和 worker 配置都可以把它们改为 `REJECT` 或 `DELETE`。`DELETE` 只写入 `DELETED` 状态，不物理删除数据库行。

### 6.5 Query Planner

自动闭环在调用来源前会先为每个有效假设生成查询计划。查询计划由 `src/server/services/internal/query-planner.ts` 提供，输出字段包括：

- `query`：实际给 adapter 使用的检索词。
- `strategy`：`LLM`、`MANUAL`、`RULE_BASE`、`RULE_COMPARISON` 或 `SETTLEMENT`。
- `purpose`：通用搜证或预测市场查询。
- `sourceKinds`：建议匹配的来源类型。

当前策略：

1. 如果假设到期需要结算，生成 `SETTLEMENT` 查询并允许所有查询类来源。
2. 如果假设配置了 `evidenceSearchQuery`，优先使用人工查询，避免规则覆盖用户意图。
3. 如果命题是比较型表达式，例如 `GPT-6 > Claude Mythos` 或 `A outperforms B`，拆出 benchmark、comparison 和预测市场查询。
4. 其他情况使用清洗后的基础查询。
5. 如果未来接入 LLM Query Planner 且输出失败或为空，自动回退到上述确定性规则。

Query Planner 的目标是提高召回率，不替代后续 LLM 相关性判断。规则未覆盖时仍会退回基础查询，而不是静默放弃采集。
...
服务层位于 `src/server/services`，围绕 `WorldModelStore` 持久化抽象构建（接口见 `types.ts`，两套实现：
`in-memory-store.ts` 与 `prisma-store.ts`），实现依赖倒置：业务逻辑只依赖接口，不直接依赖 Prisma。

### 7.1 组合根与服务分组

`world-model-services.ts` 的 `createWorldModelServices(store, options)` 是服务层的**组合根**，
按领域聚合返回 8 组服务对象（对外类型见 `types.ts` 的 `WorldModelServices`）：

- `beliefs`：创建/更新/归档信念，创建/更新/结算假设，校验概率结构，生成假设推荐。
- `observations`：写入观察、去重、标记未知/重复/拒绝/软删除/确认、结算；低影响和未知观察支持按队列批量拒绝或软删除。
- `evidence`：观察确认为证据、证据-假设关联、编辑/重应用/拒绝/软删。
- `likelihood`：调用 estimator、保存 likelihood run、返回 ensemble 结果。
- `updates`：生成更新预览、应用贝叶斯更新、保存更新事件、回滚与 rebase。
- `sources`：来源配置与 preset、运行采集 adapter、Query Planner、保存 observation run、失败/低增量来源抑制、观察清理策略。
- `automation`：证据自动闭环（采集→去重→候选→确认→更新）、心跳、worker 配置和进程内 worker 装配。
- `models`：模型产物管理与导入（含 demo/未训练产物拦截）。

### 7.2 内部 helper 模块（`src/server/services/internal/`）

跨服务复用的纯/半纯逻辑从组合根中下沉到 `internal/`，使组合根聚焦在装配与状态化流程：

- `schemas.ts`：所有 Zod 输入校验 schema 与 `parseBeliefInput`。
- `shared.ts`：调参常量 + 基础 helper（`now`、假设有效性/结算判定、文本相似度、阈值钳制）。无服务依赖，可被任意服务模块 import。
- `recommendations.ts`：假设推荐引擎（校准修复、观察驱动、分类模板）。
- `evidence-queries.ts`：证据闭环的纯查询构造与优先级计算（检索词压缩、证据覆盖聚合、校准压力、按假设排序）。
- `query-planner.ts`：把假设命题和人工查询转换成适合不同来源类型的检索计划。
- `model-artifact.ts`：模型产物导入守卫（拦截 demo/未训练产物）。

### 7.3 约束与后续拆分

- 服务层必须进行输入校验（Zod），避免 UI 或 API 直接写入不合法概率、状态或 JSON。
- LLM 主评分、轻量 fallback 加载、estimator 装配在 `src/server/models/estimators.ts`，由 `configured.ts`
  注入到组合根的 `options`。
- per-service 工厂模块已经落地，新增业务应继续放进对应聚合，避免让组合根重新膨胀。
- `internal/` helper 只能承载跨服务复用的纯/半纯逻辑；如果 helper 继续长大，应优先按职责再拆分。
...
- `POST /api/automation/evidence-loop`
- `GET /api/automation/worker`
- `POST /api/automation/worker`
- `POST /api/automation/worker/restore`
...
- `/admin/world-model/observations`
  - 观察池、重复候选、未知证据、确认、拒绝和软删除操作；重复候选可链接原始观察并批量拒绝/删除，低影响和未知观察可按队列批量拒绝/删除。
- `/admin/world-model/evidence`
  - 证据库、假设关联、更新预览、应用更新。
- `/admin/world-model/sources`
  - 来源配置、可信度、adapter、自动确认阈值、证据质量反馈、一键应用校准建议、手动运行、自动闭环和 worker 清理策略。
